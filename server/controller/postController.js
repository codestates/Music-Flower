...
module.exports = {
  ...
  createPost: async (req, res) => {
    const {userId, postTitle, image, postExplain, createdAt, musicList} = req.body;
    ...
    .then(result => {
     ...
     //key값 관련해서 오류 있을 수 있음. 
      const bulkList = musicList.map((el)=> { result.id, el});
  ...
  updatePost: async (req, res) => {
    ...
    .then (() => {
    const bulkList = musicList.map((el)=> { postId, el});
    Post_MusicData.bulkcreate(bulkList)
    });
